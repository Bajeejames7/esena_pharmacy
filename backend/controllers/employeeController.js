...
exports.createEmployee = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO users (username, email, full_name, phone, password, role, status, otp, otp_expires)
       VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
      [username, email, full_name, phone || null, placeholder, role, otp, otpExpires]
    );
    ...
    await sendEmail({
      to: email,
      subject: 'Welcome to Esena Pharmacy - Set Up Your Account',
      html: `
        ...
        <div class="container">
          ...
          <div class="content">
            <p>Hi ${full_name},</p>
            <p>An admin has created an account for you. Use the one-time password below to log in and set up your password.</p>
            <div class="otp-box">
              <p style="margin:0;color:#666;font-size:14px">Your One-Time Password</p>
              <div class="otp">${otp}</div>
              <p style="margin:0;color:#999;font-size:12px">Expires in 24 hours</p>
            </div>
            <p><strong>Login URL:</strong> <a href="${frontendUrl}/admin/login">${frontendUrl}/admin/login</a></p>
            <p><strong>Username:</strong> ${username}</p>
            <p>After logging in with this OTP, you will be prompted to create a new password.</p>
    ...
    res.status(201).json({ message: 'Employee created and invitation sent', id: result.insertId });
  } catch (error) {
...
exports.getActivityLog = async (req, res) => {
  try {
    const { resource_type, resource_id, user_id, limit = 100 } = req.query;
