-- ============================================================
-- ESENA PHARMACY - COMPLETE DATABASE SCHEMA
-- Last Updated: 2026-07-10
-- ============================================================
-- This file represents the complete, current database schema
-- including all features: orders, customers, drivers, M-Pesa, etc.
-- ============================================================
...
-- Password hash: $2a$10$rZ5qH8qF9xK3yL7mN9pO4.xJ5vK8wL9mN6pO4xJ5vK8wL9mN6pO4x
INSERT INTO users (username, password, role) VALUES 
('admin', '$2a$10$rZ5qH8qF9xK3yL7mN9pO4.xJ5vK8wL9mN6pO4xJ5vK8wL9mN6pO4x', 'admin')
...
-- Customer Accounts with health information and profile completion
...
CREATE TABLE IF NOT EXISTS stock_movements (
  ...
  quantity INT NOT NULL COMMENT 'Positive for stock in, negative for stock out',
  movement_type ENUM('purchase','sale','adjustment','return') NOT NULL,
  reference_id INT COMMENT 'Order ID or purchase ID',
  notes TEXT,
  created_by INT COMMENT 'User ID who created this movement',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS orders (
  id INT AUTO_INCREMENT PRIMARY KEY,
  customer_id INT DEFAULT NULL COMMENT 'Links to customers table if logged in',
  customer_name VARCHAR(255) NOT NULL,
  email VARCHAR(255),
  phone VARCHAR(20),
  delivery_address TEXT,
  notes TEXT COMMENT 'Payment method notes or special instructions',
  ...
  total DECIMAL(10,2),
  token VARCHAR(50) UNIQUE NOT NULL COMMENT 'Public tracking token',
  status ENUM('pending','payment_requested','paid','dispatched','out_for_delivery','ready_for_pickup','completed','cancelled') DEFAULT 'pending',
  payment_method ENUM('cash','mpesa','bank_transfer') DEFAULT 'cash',
  mpesa_receipt VARCHAR(50) COMMENT 'M-Pesa receipt number',
  handled_by INT DEFAULT NULL COMMENT 'Admin who processed this order',
  handled_by_name VARCHAR(255) DEFAULT NULL COMMENT 'Name of admin for display',
  current_delivery_id INT DEFAULT NULL COMMENT 'Current active delivery assignment',
  driver_assigned_at TIMESTAMP NULL COMMENT 'When driver was assigned',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  INDEX idx_current_delivery (current_delivery_id),
  INDEX idx_created_at (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS mpesa_payments (
  ...
  expires_at DATETIME COMMENT 'Payment request expiration time',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE CASCADE,
...
-- Deliveries (tracks order assignments to drivers)
...
CREATE TABLE IF NOT EXISTS appointments (
  ...
  handled_by INT DEFAULT NULL COMMENT 'Doctor/admin who handled appointment',
  handled_by_name VARCHAR(255) DEFAULT NULL COMMENT 'Name of doctor for display',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE SET NULL,
  INDEX idx_token (token),
...
-- Prescriptions (legacy table - may be merged with appointment_notes)
...
-- Activity Log (comprehensive audit trail for all system actions)
...
-- Audit Logs (legacy - may be merged with activity_log)
